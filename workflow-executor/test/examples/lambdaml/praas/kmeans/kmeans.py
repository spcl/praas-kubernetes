...
from . import libsvm_dataset, cluster_models
...
def merge_epoch(local_data, epoch, worker_index, n_workers, name):
    ...
    praassdk.msg.broadcast(others, local_slice_key, local_data)

    for i in range(n_workers):
        if i == worker_index:
            continue
        msg_id = f"{name}-{i}-{epoch}"
        remote_slice = praassdk.msg.get(i, msg_id)
...
def kmeans(args):
    ...
    if worker_index == 0:
        ...
        for other in range(1, n_workers):
            praassdk.msg.put(other, init_msg_name, centroids)  # TODO(gr): use broadcast
        op = "gen_centroid"
        output("generate initial centroids takes {} s"
               .format(time.perf_counter() - init_centroids_start))
    else:
        centroids = praassdk.msg.get(0, init_msg_name)
        op = "wait_centroid"
