...
func CreateAppWithProcesses(
	...
	appID, err := clients.Praas.CreateApp()
	if err != nil {
		return -1, err
	}

	ids, pods, err := clients.Praas.CreateProcesses(appID, numProcesses)
	if sessionIDs != nil {
		*sessionIDs = ids
	}
	if podToPID != nil {
		for i := 0; i < numProcesses; i++ {
			podToPID[pods[i]] = ids[i]
		}
	}
	...
	isAtScale := false
	for !isAtScale {
		scale, err := clients.Praas.GetScaleForApp(appID, true)
		if err == nil {
			isAtScale = scale == numProcesses
		}
